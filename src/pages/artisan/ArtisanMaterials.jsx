...
export default function ArtisanMaterials() {
  ...
  function handleSubmit(e) {
    e.preventDefault()
    const valid = lines.filter((l) => l.specification.trim() && l.quantity && l.location.trim() && l.requiredDate)
  ...
  return (
    <div className="page page-narrow">
      <Stepper steps={['Your details', 'Material needs', 'Match & buy', 'Confirm', 'Track']} current={1} />
      ...
      <form onSubmit={handleSubmit}>
        {lines.map((line) => (
          <div className="line-item-card" key={line.key}>
            {lines.length > 1 && (
              <button type="button" className="line-item-remove" onClick={() => removeLine(line.key)}>✕</button>
            )}
            <div className="field-row">
              ...
              <div className="field">
                ...
                <input value={line.specification} onChange={(e) => updateLine(line.key, 'specification', e.target.value)} placeholder="e.g. Muga silk yarn, 20/22 denier" />
              </div>
            </div>
            <div className="field-row">
              <div className="field">
            ...
            <div className="field">
              ...
              <input value={line.location} onChange={(e) => updateLine(line.key, 'location', e.target.value)} placeholder="e.g. Sualkuchi, Assam" />
            </div>
          </div>
        ))}

        <button type="button" className="btn btn-outline" onClick={addLine} style={{ marginBottom: '1.5rem' }}>
          {t('addAnotherLine')}
        </button>

        <button type="submit" className="btn btn-primary btn-block">{t('findMatchesBtn')}</button>
      </form>
    </div>
  )
}
